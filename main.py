...
def winning_move(board, piece, pos):
    ...
    for row in range(len(board)):
        for column in range(len(board)-2):
            if board[row][column] == piece and board[row][column+1] == piece and board[row][column+2] == piece:
                pos['start_pos'] = (10, row * square + square/2)
                pos['end_pos'] = (width-10, row * square + square/2)
                return True
    ...
    for row in range(len(board)-2):
        for column in range(len(board)):
            if board[row][column] == piece and board[row+1][column] == piece and board[row+2][column] == piece:
                pos['start_pos'] = (column * square + square/2, 10)
    ...
    for row in range(len(board)-2):
        for column in range(len(board)-2):
            if board[row][column] == piece and board[row+1][column+1] == piece and board[row+2][column+2] == piece:
                pos['start_pos'] = (column * square + 20, row * square + 20)
                pos['end_pos'] = ((column+2) * square + square -20, (row+2) * square + square -20)
                return True
    ...
    for row in range(2, len(board)):
        for column in range(len(board)-2):
            if board[row][column] == piece and board[row-1][column+1] == piece and board[row-2][column+2] == piece:
                pos['start_pos'] = (column * square + 20, row * square + square -20)
...
def draw_grid(board):
    for row in range(len(board)-1):
        # Horizontal lines
        pygame.draw.line(win, (128,128,128), (0, square + row * square), (width, square + row * square), 5)
        for column in range(len(board)-1):
            # Vertical lines
            pygame.draw.line(win, (128,128,128), (square + row * square, 0), (square + row * square, height), 5)

def draw_board(board):
    for row in range(len(board)):
        for column in range(len(board)):
            pygame.draw.rect(win, (0,0,0), (column * square, row * square, square, square))
